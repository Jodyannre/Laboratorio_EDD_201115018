...
template <class T>
int listaGenerica<T>::getTamano(){
    return this->tamano;
}
template <class T>
void listaGenerica<T>::imprimirCopia(nProyecto* actual){
    if (actual == NULL){
        return;
    }
    if (actual!= NULL){
        cout<<actual->getValor()<<") "<<actual->getDato()->getNombre()<<endl;
    }
    imprimirCopia(actual->getSig());
    return;
}

template class listaGenerica<nProyecto>;